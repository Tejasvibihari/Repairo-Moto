...
export default function AdminReferralDashboard() {
    ...
    const handleUpdateRequest = async (e) => {
        ...
        try {
            // Mock API call
            setTimeout(() => {
                // Update the request in the user data
                const updatedRequests = user.withdrawalRequests.map(req =>
                    req._id === selectedRequest._id
                        ? { ...req, status, adminNote, transactionId, processedDate: status === 'approved' || status === 'paid' ? new Date().toISOString() : req.processedDate }
                        : req
                );
                setUser({ ...user, withdrawalRequests: updatedRequests });
                closeModal();
                setLoading(false);
            }, 1000);
        } catch (error) {
            setError('Error updating withdrawal request');
            console.error(error);
            setLoading(false);
        }
    };
