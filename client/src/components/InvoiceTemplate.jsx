      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div id="invoice-container" className="bg-white shadow-lg rounded-lg p-8">
          ...
          <div className="bg-yellow-50 p-4 rounded-md mb-6 border-l-4 border-yellow-500">
            <p className="text-yellow-800 font-medium">Order ID: {invoiceData.orderId}</p>
          ...
          <table className="w-full mb-6">
            ...
            <tbody>
              {invoiceData.partsUsed && invoiceData.partsUsed.map((part, index) => {
                ...
                // Determine the per unit discount based on the discountType for each part
                const discountPerUnit = part.discountType === 'percentage'
                  ? (price * part.discountPrice) / 100 // Calculate percentage discount
                  : part.discountPrice; // Use the fixed amount discount

                // Calculate total discount and final price
                const totalDiscount = discountPerUnit * quantity;
                const finalPrice = (price * quantity) - totalDiscount;
                ...
                return (
                  <tr key={part._id.$oid} className={index % 2 === 1 ? 'bg-yellow-50' : ''}>
                    ...
                    <td className="border-b border-gray-200 py-3 px-4">{quantity}</td>
                    <td className="border-b border-gray-200 py-3 px-4">₹{price.toFixed(2)}</td>
                    <td className="border-b border-gray-200 py-3 px-4">
                      {part.discountType === 'percentage'
                        ? `${part.discountPrice}%` // Show percentage discount
                        : `₹${part.discountPrice.toFixed(2)}`} {/* Show fixed amount discount */}
                    </td>
                    <td className="border-b border-gray-200 py-3 px-4">₹{totalDiscount.toFixed(2)}</td>
                    <td className="border-b border-gray-200 py-3 px-4 font-semibold">₹{finalPrice.toFixed(2)}</td>
                  </tr>
                );
              })}
            </tbody>
