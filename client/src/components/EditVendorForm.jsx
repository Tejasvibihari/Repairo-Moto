...
export default function EditVendorForm({ initialData }) {
    const [formData, setFormData] = useState({
        ...
        businessName: initialData?.businessName || '', // New field
        gstNo: initialData?.gstNo || '', // New field
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const response = await axiosClient.put(`/api/vendor/updatevendor/${initialData._id}`, formData,
                {
                    headers: {
    ...
    return (
        <div>
            ...
            <form className="shadow-sm border border-gray-300 rounded p-4 mt-4" onSubmit={handleSubmit}>
                ...
                >
                    {loading ? <CircularLoading size={25} /> : <span className='flex flex-row'><Store className='mr-2' />Edit Vendor</span>}
